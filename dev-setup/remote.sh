...
case "$COMMAND" in
  up)
    ...
    dns_provider_type=$(yq -e 'select(document_index == 1) | .metadata.annotations.["dns.gardener.cloud/provider"]' "$SCRIPT_DIR"/gardenconfig/overlays/remote/credentials/domains/domain-secrets.yaml)

  yq -e -i "
    .spec.config.seedConfig.spec.dns.provider.secretRef.name = \"$internal_dns_secret\" |
    .spec.config.seedConfig.spec.dns.provider.type = \"$dns_provider_type\"
  " "$SCRIPT_DIR"/gardenlet/overlays/remote/gardenlet.yaml
    ...
    if kubectl get configmaps -n kube-system shoot-info -o yaml > "$temp_shoot_info"; then
      ...
      yq -e -i "
        .spec.runtimeCluster.networking.pods = \"$pods_cidr\" |
        .spec.runtimeCluster.networking.nodes = \"$nodes_cidr\" |
        .spec.runtimeCluster.networking.services = \"$services_cidr\" |
    ...
    check-not-initial "$SCRIPT_DIR"/gardenlet/overlays/remote/gardenlet.yaml ".spec.config.seedConfig.spec.dns.provider.secretRef.name"
