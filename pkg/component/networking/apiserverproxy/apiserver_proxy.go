...
func (a *apiserverProxy) computeResourcesData() (map[string][]byte, error) {
	...
	xGardenerDestination := fmt.Sprintf("outbound|443||kube-apiserver.%s.svc.cluster.local", a.namespace)
	if a.values.IstioTLSTermination {
		xGardenerDestination = apiserverexposure.GetAPIServerProxyTargetClusterName(a.namespace)
	}
	...
	if err := tplEnvoy.Execute(&envoyYAML, map[string]any{
		...
		"xGardenerDestination": xGardenerDestination,
	}); err != nil {
		return nil, err
	}
